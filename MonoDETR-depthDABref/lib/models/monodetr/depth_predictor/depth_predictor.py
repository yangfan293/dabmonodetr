...
class DepthPredictor(nn.Module):
    ...
    def __init__(self, model_cfg):
        ...
        # Create modules
        d_model = model_cfg["hidden_dim"]  # hidden_dim=256
    ...
    def forward(self, feature, mask, pos):
        ...
        src = self.depth_head(src)  # depth_head()为两个3X3的卷积,获取fD

        depth_logits = self.depth_classifier(src)  # depth_classifier()为一个1X1的卷积，获得深度值对应的bin值
        depth_probs = F.softmax(depth_logits, dim=1)  # 对depth_logits进行softmax，得到每一个Bin值对应的概率
        weighted_depth = (depth_probs * self.depth_bin_values.reshape(1, -1, 1, 1)).sum(dim=1)
        # 对每一个Bin值对应的概率*Bin值并加和，获得此类目标的深度值 16,24,80

        # depth embeddings with depth positional encodings
        B, C, H, W = src.shape
        src = src.flatten(2).permute(2, 0, 1)  # 降维并转置
        mask = mask.flatten(1)
        pos = pos.flatten(2).permute(2, 0, 1)

        depth_embed = self.depth_encoder(src, mask, pos)  # 对深度特征src进行多头注意力机制计算
        depth_embed = depth_embed.permute(1, 2, 0).reshape(B, C, H, W)  # 进行转置变形等操作获得深度编码

        depth_pos_embed_ip = self.interpolate_depth_embed(weighted_depth)  # 深度值对应的位置编码 16,256,24,80
        depth_embed = depth_embed + depth_pos_embed_ip  # 与tansformer的输入一样，特征编码+位置编码
    ...
    def interpolate_1d(self, coord, embed):
        ...
        ceil_coord = (floor_coord + 1).clamp(max=embed.num_embeddings - 1)  # 若有框重叠，取最近的
