    ...
    def forward(self, images, calibs, targets, img_sizes):
        ...
        if self.num_feature_levels > len(srcs):
            ...
            for l in range(_len_srcs, self.num_feature_levels):
                ...
                backbone_1 = self.backbone[1]
                pos_l = backbone_1(NestedTensor(src, mask)).to(src.dtype)
                srcs.append(src)
                masks.append(mask)
                pos.append(pos_l)
        ...
        if not self.two_stage:
            query_embeds = self.query_embed.weight  # nq,256+3

        pred_depth_map_logits, depth_pos_embed, weighted_depth = self.depth_predictor(srcs, masks[1], pos[1])
        # pred_depth_map_logits(16,81,24,80) depth_pos_embed(16,256,24,80) weighted_depth(16,24,80)
        hs, learnable_anchor_box_3d, inter_references_dim, depth_reg, enc_outputs_class, enc_outputs_coord_unact = self.depthaware_transformer(
            srcs, masks, pos, query_embeds, depth_pos_embed, weighted_depth)
        # hs:tezheng,init_reference:transformer裏最開始的編碼,
        # inter_references:經過decoder網絡更新的參考點,6維時是xy,t,b,l,r,inter_references_dim:3D框的gao長寬
        outputs_coords = []
        outputs_classes = []
        outputs_3d_dims = []
        outputs_depths = []
        ...
        for lvl in range(hs.shape[0]):
            if lvl == 0:
                reference = query_embeds[..., :2]
            else:
                reference = learnable_anchor_box_3d[lvl - 1][..., :6]
            # depth_map = learnable_anchor_box_3d[lvl - 1][..., 2:3]
            # depth_map = inverse_sigmoid(depth_map)
            reference = inverse_sigmoid(reference)

            tmp = self.bbox_embed[lvl](hs[lvl])  # x,y,z,l,w,h
            ...
            # depth average + sigma
            depth_ave = torch.cat(
                [((1. / (depth_reg2[:, :, 0: 1].sigmoid() + 1e-6) - 1.) + depth_geo.unsqueeze(-1) + depth_map) / 3,
                 depth_reg2[:, :, 1: 2]], -1)
